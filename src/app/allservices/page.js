...
const AllGigs = () => {
  ...
  const [completedSwapCounts, setCompletedSwapCounts] = useState({}); // Changed name to be more specific
      ...
      try {
        ...
        await Promise.all(
          gigList.map(async (gig) => {
            ...
            // Fetch completed swap count using the same logic as dashboard
            if (!completedSwapCountsData[gig.username]) {
              const completedCount = await getCompletedSwapsForUser(
                gig.username
              );
              completedSwapCountsData[gig.username] = completedCount;
            }
          })
        );

        setProfiles(profilesData);
        setCompletedSwapCounts(completedSwapCountsData);
        setGigs(gigList);
                      ...
                      <button
                        ...
                        className={`!px-4 !py-2 !rounded-full !text-sm !font-medium !transition-all !whitespace-nowrap !flex-shrink-0 ${
                          selectedCategory === category
                            ...
                            : "!bg-white !text-gray-600 !border-2 !border-green-500 hover:!border-green-600"
                        }`}
                      >
                        {category}
                      </button>
        ...
        {filteredGigs.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredGigs.map((gig) => {
              ...
              const completedSwapCount = completedSwapCounts[gig.username] ?? 0; // Changed variable name
                ...
                >
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    <div className="relative">
                      ...
                      {/* Success Badge - Only show if there are completed swaps */}
